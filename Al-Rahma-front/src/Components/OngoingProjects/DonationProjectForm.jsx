...
const DonationProjectForm = ({ 
  ...
  useEffect(() => {
    if (project) {
      ...
      const date = new Date();
      const reference = `DON-${project.projectCode}-${date.getFullYear()}${(date.getMonth() + 1).toString().padStart(2, '0')}${date.getDate().toString().padStart(2, '0')}`;
      ...
      setFormData(prev => ({
        ...
        description: `تبرع مرتبط بالمشروع: ${project.project}`,
  ...
  const handleInputChange = (e) => {
    ...
    setFormData(prev => {
      ...
      if (name === 'cashAmount' && prev.suppliesType === 'نقدي وعيني') {
        newData.monetaryValue = calculateTotal(prev.items) + parseFloat(value || 0);
      } else if (name === 'cashAmount' && prev.suppliesType === 'نقدي') {
  ...
  const validateForm = () => {
    ...
    if (['عيني', 'نقدي وعيني'].includes(formData.suppliesType) && formData.items.length === 0) {
      newErrors.items = 'يجب إضافة عنصر واحد على الأقل';
    }

    // Monetary value validation for cash donations
    if (['نقدي', 'نقدي وعيني'].includes(formData.suppliesType) && formData.cashAmount <= 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('MonetaryValue', formData.monetaryValue.toString());
      ...
      alert('تم إنشاء التبرع للمشروع بنجاح!');
      onHide();
      
    } catch (error) {
      console.error('Error creating project donation:', error);
  ...
  const handleRemoveItem = (index) => {
    ...
    setFormData(prev => {
      ...
      // Update monetaryValue based on donation type
      if (prev.suppliesType === 'عيني') {
        newData.monetaryValue = calculateTotal(newItems);
      } else if (prev.suppliesType === 'نقدي وعيني') {
    ...
    <>
      <Modal show={show} onHide={onHide} size="xl" centered>
        <Modal.Header closeButton>
          <Modal.Title>إضافة تبرع للمشروع: {project?.project}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <Row className="mb-3">
              ...
              <Form.Group as={Col} md={3} controlId="suppliesType">
                <Form.Label>نوع التبرع *</Form.Label>
                <Form.Select
                  name="suppliesType"
                  value={formData.suppliesType}
                  onChange={handleDonationTypeChange}
                  isInvalid={!!errors.suppliesType}
                >
              ...
              <Form.Group as={Col} md={3} controlId="suppliesScope">
                <Form.Label>نطاق التبرع *</Form.Label>
                <Form.Select
                  name="suppliesScope"
                  value={formData.suppliesScope}
                  onChange={handleInputChange}
                  isInvalid={!!errors.suppliesScope}
                >
            ...
            <Row className="mb-3">
              <Form.Group as={Col} md={4} controlId="cashAmount">
                ...
                <Form.Control
                  ...
                  disabled={formData.suppliesType === 'عيني'}
                  min="0"
                  step="0.01"
                />
                <Form.Control.Feedback type="invalid">
            ...
            {['عيني', 'نقدي وعيني'].includes(formData.suppliesType) && (
              <div className="mb-4">
                <h5>العناصر</h5>
...
export default DonationProjectForm;